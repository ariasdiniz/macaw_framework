...
class CacheAspectTest < Minitest::Test
  def setup
    ...
    @cache_obj = {
      ...
      cached_methods: { 'endpoint1' => [:allowed_header, :allowed_param] }
    }
    @client_data = {
  ...
  def test_no_cache_when_cache_nil
    cache = @cache_obj.dup
    cache[:cache] = nil
    @dummy.set_response(["result_no_cache", 200])
  ...
  def test_cache_miss_and_store_successful_response
    cache = @cache_obj.dup
    @dummy.set_response(["fresh_result", 200])
    result = @dummy.call_endpoint(cache, 'endpoint1', @client_data)
    ...
    assert cache[:cache].cache.has_key?(expected_key)
  ...
  def test_cache_miss_no_store_on_unsuccessful_response
    cache = @cache_obj.dup
    @dummy.set_response(["error_result", 404])
    result = @dummy.call_endpoint(cache, 'endpoint1', @client_data)
    ...
    filtered = { params: { allowed_param: 'param1' }, headers: { allowed_header: 'value1' } }
    expected_key = [filtered].to_s.to_sym
    refute cache[:cache].cache.has_key?(expected_key)
  end
  ...
  def test_kwargs_are_passed_to_super
    cache = @cache_obj.dup
    @dummy.set_response(["result_kw", 200])
    kwargs = { key: 'value' }
  ...
  def test_cache_boundary_status_299
    cache = @cache_obj.dup
    @dummy.set_response(["boundary_result", 299])
    result = @dummy.call_endpoint(cache, 'endpoint1', @client_data)
    filtered = { params: { allowed_param: 'param1' }, headers: { allowed_header: 'value1' } }
    expected_key = [filtered].to_s.to_sym
    assert cache[:cache].cache.has_key?(expected_key)
  ...
  def test_cache_boundary_status_300
    cache = @cache_obj.dup
    @dummy.set_response(["non_cache_result", 300])
    result = @dummy.call_endpoint(cache, 'endpoint1', @client_data)
    filtered = { params: { allowed_param: 'param1' }, headers: { allowed_header: 'value1' } }
    expected_key = [filtered].to_s.to_sym
    refute cache[:cache].cache.has_key?(expected_key)
